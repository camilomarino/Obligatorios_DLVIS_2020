...
def discriminator_loss(logits_real, logits_fake):
    ...
    target_real = torch.ones(logits_real.shape[0]).type(dtype)
    target_fake = torch.zeros(logits_fake.shape[0]).type(dtype)
    
    loss = bce_loss(logits_real, target_real) + bce_loss(logits_fake, target_fake)
...
def generator_loss(logits_fake):
    ...
    target_fake = torch.ones(logits_fake.shape[0]).type(dtype)    
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential( #28x28
                    #Unflatten(C=1, H=28, W=28),
                    nn.Conv2d(1, 32, (5,5)), #24x24
                    nn.LeakyReLU(0.01),
                    nn.MaxPool2d((2,2), stride=2), #12x12
                    nn.Conv2d(32, 64, (5,5)), #8x8
                    nn.LeakyReLU(0.01),
                    nn.MaxPool2d((2,2), stride=2), #4x4
                    Flatten(),
